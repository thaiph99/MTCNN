...
def gen_hard_bbox_pnet(srcDataSet, srcAnnotations):
    ...
    for annotation in annotationsFile:
        ...
        print("height %d width %d channel %d\n" % (height, width, channel))
...
def gen_hard_bbox_pnet_plate(srcDataSet, srcAnnotations):
    srcDataSet = os.path.join(rootPath, srcDataSet)
    srcAnnotations = os.path.join(rootPath, srcAnnotations)
    saveFolder = os.path.join(rootPath, "tmp/data/pnet/")
    print(">>>>>> Gen hard samples for pnet...")
    ...
    for annotation in annotationsFile:
        ...
        boxes = np.array(bbox, dtype=np.float32).reshape(-1, 4)
        # load image
        img = cv2.imread(os.path.join(srcDataSet, imPath))
        idx += 1
        height, width, channel = img.shape
        ...
        for box in boxes:
            ...
            posNum = 0
            partNum = 0
            while posNum < 10 or partNum < 10:
                # pos and part plate size
                size_w = np.random.randint(int(w * 0.8), np.ceil(1.25 * w))
                size_h = np.random.randint(int(h * 0.8), np.ceil(1.25 * h))
                # delta here is the offset of box center
                delta_x = np.random.randint(-w * 0.2, w * 0.2)
                delta_y = np.random.randint(-h * 0.2, h * 0.2)
                # show this way: nx1 = max(x1+w/2-size/2+delta_x)
                ...
                # print(iou,posNum,partNum)
                if posNum < 10 and is_pos_sample(x1, y1, x2, y2, nx1, ny1, nx2, ny2) and iou >= 0.65:
                    save_file = os.path.join(saveFolder, "pos", "%s.jpg" % pIdx)
                    saveFiles['pos'].write(save_file + ' 1 %.2f %.2f %.2f %.2f\n' %
                                           (offset_x1, offset_y1, offset_x2, offset_y2))
                    cv2.imwrite(save_file, resized_im)
                    pIdx += 1
                    posNum += 1
                elif partNum < 10 and iou < 0.65 and iou >= 0.4:
                    save_file = os.path.join(saveFolder, "part", "%s.jpg" % dIdx)
                    saveFiles['part'].write(save_file + ' -1 %.2f %.2f %.2f %.2f\n' %
                                            (offset_x1, offset_y1, offset_x2, offset_y2))
                    cv2.imwrite(save_file, resized_im)
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Generate Bbox data for PNet',
...
if __name__ == "__main__":
    ...
    if args.mydata == False:
        ...
        if args.lmnum == 5:
            gen_hard_bbox_pnet("dataset/", "dataset/anno_file.txt")
        elif args.lmnum == 4:
            gen_hard_bbox_pnet_plate("dataset/", "dataset/anno_file.txt")
