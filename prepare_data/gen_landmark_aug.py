...
def gen_landmark_data(srcTxt, net, augment=False):
    ...
    for (imgPath, bbox, landmarkGt) in getBboxLandmarkFromTxt(srcTxt,5):
        F_imgs = []
        F_landmarks = []        
...
def gen_landmark_data_plate(srcTxt, net, augment=False):
    '''
    ...
    saveF = open(join(saveFolder, "landmark.txt"), 'w')
    imageCnt = 0
    # image_path bbox landmark(4*2)
    for (imgPath, bbox, landmarkGt) in getBboxLandmarkFromTxt(srcTxt,4):
        F_imgs = []
        F_landmarks = []        
        img = cv2.imread(imgPath)
        assert(img is not None)
        ...
        if augment:
            ...
            #gt's height
            gt_h = y2 - y1 + 1        
            if max(gt_w, gt_h) < 30 or x1 < 0 or y1 < 0:
                continue
...
if __name__ == "__main__":
    ...
    if args.mydata == False:
        ...
        if args.lmnum == 5:
            gen_landmark_data("dataset/anno_file.txt", stage, augment=True)
        elif args.lmnum == 4:
            gen_landmark_data_plate("dataset/anno_file.txt", stage, augment=True)
