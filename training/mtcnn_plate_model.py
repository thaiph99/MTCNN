...
def prelu(inputs):
    alphas = tf.get_variable("alphas", shape=inputs.get_shape(
    )[-1], dtype=tf.float32, initializer=tf.constant_initializer(0.25))
...
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        # batch*H*W*2
        conv4_1 = slim.conv2d(net, num_outputs=2, kernel_size=[
                              1, 13], stride=1, scope='conv4_1', activation_fn=tf.nn.softmax)
        ...
        # batch*H*W*8
        landmark_pred = slim.conv2d(net, num_outputs=8, kernel_size=[
                                    1, 13], stride=1, scope='conv4_3', activation_fn=None)
